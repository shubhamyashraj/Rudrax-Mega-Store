...
import { CheckCircle, Clock, Package, RefreshCw, Send, ShieldAlert, Sparkles, Star, Tag, XCircle } from 'lucide-react';
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-gradient-to-r from-teal-800 to-teal-950 text-white rounded-3xl p-6 shadow-md mb-8 flex justify-between items-center flex-wrap gap-4 relative overflow-hidden select-none">
          ...
          <div className="flex items-center gap-4">
            {currentUser.photoURL ? (
              <img src={currentUser.photoURL} className="h-14 w-14 rounded-2xl border border-teal-500 object-cover shadow-sm" referrerpolicy="no-referrer" />
            ) : (
        ...
        {returnOrderId && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl border border-slate-200 max-w-md w-full p-6 shadow-xl animate-scaleIn">
              <h3 className="text-base font-black text-slate-900 mb-3 flex items-center gap-1.5 leading-tight">
                ...
                <div key={ord.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm">
                      ...
                      {ord.items.map((item, idx) => {
                        ...
                        return (
                          <div key={idx} className="flex gap-2.5 items-center text-xs">
                            <img src={pr?.image} className="h-8 w-8 object-cover rounded border border-slate-100" />
                  ...
                  <div className="pt-4">
                    ...
                    <div className="flex items-start md:flex-row flex-col gap-4 justify-between w-full relative">
                      ...
                      ].map((tlNode, index, arr) => {
                        // Figure out if this checkpoint is achieved, in progress, or pending
                ...
                return (
                  <div key={ret.id} className="bg-white border border-slate-200 rounded-2xl p-5 flex gap-4 hover:shadow-sm">
                    <div className="h-14 w-14 rounded-lg bg-slate-50 flex items-center justify-center p-1 border border-slate-100 flex-shrink-0">
                      <img src={targetPr?.image} className="object-cover h-full w-full rounded" />
                    </div>
