...
import { ArrowLeft, Check, Compass, CreditCard, Home, MapPin, Phone, Plus, ShieldCheck, ShoppingBag, Smartphone, Truck } from 'lucide-react';
...
export function CheckoutFlow() {
  ...
  const [orderCoupon, setOrderCoupon] = useState<string>('RUDRAX20'); // Quick seed coupon to make flow extremely easy
  const [hasCouponFlag, setHasCouponFlag] = useState<boolean>(true);
              ...
              {showAddAddrForm && (
                <form onSubmit={handleAddNewAddress} className="bg-white border border-slate-200 rounded-2xl p-5 flex flex-col gap-3.5 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-35">
                    ...
                    <Input label="Mobile Number" placeholder="e.g. +91 99999 88888" value={newPhone} onChange={setNewPhone} />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <Input label="City" placeholder="Gurugram" value={newCity} onChange={setCityValue => setNewCity(setCityValue)} />
                    <Input label="State" placeholder="Haryana" value={newState} onChange={setStateValue => setNewState(setStateValue)} />
                    <Input label="Postal Code" placeholder="122002" value={newZip} onChange={setZipValue => setNewZip(setZipValue)} />
                  </div>
                  ...
                  {cartWithData.map((item, index) => (
                    <div key={index} className="flex justify-between items-center text-xs">
                      <div className="flex items-center gap-2">
                        <img src={item.product?.image} className="h-8 w-8 object-cover rounded" />
                        <div>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm h-fit">
      <h3 className="text-sm font-bold text-slate-900 border-b border-slate-100 pb-3 mb-4 flex items-center gap-1.5 font-mono">
        <ShoppingBag size={16} className="text-teal-600" /> Checkout Reciept
      </h3>
