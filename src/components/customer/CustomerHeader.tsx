...
import { ShoppingCart, Search, User, ShieldAlert, Notebook, HelpCircle, FileText, Sparkles, Tag, CheckCircle } from 'lucide-react';
...
export function CustomerHeader() {
  ...
  const activeOrdersCount = orders.filter(o => o.status !== 'Completed' && o.status !== 'Returned').length;
  ...
  return (
    <header className="header flex flex-col w-full bg-white shadow-sm border-b border-slate-100 z-30 sticky top-0">
      ...
      <div className="bg-teal-900 text-white text-xs py-2 px-4 flex justify-between items-center font-medium md:flex-row flex-col gap-1.5">
      ...
      <div className="bg-slate-900 text-slate-200 text-xs px-4 py-2 flex md:justify-between justify-center items-center flex-wrap gap-2.5 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono bg-slate-800 text-slate-300 px-2 py-0.5 rounded text-[10px]">d8bbeae6-applet-run</span>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="inline-flex gap-1 bg-slate-800 p-0.5 rounded-lg border border-slate-700">
            ...
            >
              ...
              {lowStockCount > 0 && (
                <span className="bg-red-600 text-white font-bold h-4 px-1 rounded-full text-[9px] flex items-center justify-center">
                  {lowStockCount} alert
                </span>
              )}
            </button>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 md:py-4 py-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => { setActivePage('home'); setSearchQuery(''); }}>
          <div className="h-10 w-10 bg-gradient-to-tr from-teal-800 to-teal-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-teal-800/20">
            <span className="font-extrabold text-xl font-display tracking-tight">R</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xl font-extrabold text-slate-900 font-display tracking-tight leading-none flex items-center gap-1">
            ...
            <span className="text-[9px] text-slate-500 font-semibold tracking-widest uppercase mt-0.5">Multi-Category Platform</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="hidden lg:flex flex-col items-end border-r border-slate-100 pr-5">
            <div className="flex items-center gap-1 text-[11px] text-amber-600 font-bold bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200">
          ...
          >
            ...
            {totalCartCount > 0 && (
              <span className="absolute -top-1.5 -right-1.5 bg-red-600 text-white font-bold h-5 min-w-[20px] px-1 rounded-full text-[10px] flex items-center justify-center shadow-md shadow-red-200 animate-bounce">
                {totalCartCount}
              </span>
            )}
          </button>
...
function BadgeHighlight({ children }: { children: string }) {
  return (
    <span className="bg-amber-400 text-slate-950 font-bold px-2 py-0.5 rounded text-[10px] uppercase tracking-wide inline-block mr-1">
      {children}
    </span>
  );
}
